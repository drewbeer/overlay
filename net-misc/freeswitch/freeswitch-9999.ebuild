#
# Copyright (C) 2008-2014 Stefan Knoblich <s.knoblich@axsentis.de>
#
...
IUSE_MODULES="alsa amr amrwb avmd bv +cdr_csv cdr_pg_csv cdr_sqlite celt cepstral cidlookup +console curl
	...
	shell_stream shout silk siren skinny skypopen snapshot +sndfile snom +sofia +spandsp spidermonkey spy +syslog
	+tone_stream tts_commandline unimrcp v8 valet_parking vmd +voicemail
...
fs_check_core_abi_compat() {
	...
	libfreeswitch="$(ls -1 "${EROOT}"/opt/freeswitch/lib/libfreeswitch.so.*.*.*)"
...
fs_check_modules_api_compat() {
	...
	for x in "${EROOT}"/opt/freeswitch/mod/mod_*.so; do
		mod="$(basename "${x}")"
...
pkg_preinst() {
	...
	if use nosamples; then
		if has_version "net-misc/freeswitch"; then
			...
			cp -dpR "${EROOT}/etc/freeswitch" "${D}/etc"
		fi
	fi
}
